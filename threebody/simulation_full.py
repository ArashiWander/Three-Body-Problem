...
def main():
    ...
    control_panel = pygame_gui.elements.UIPanel(
        relative_rect=control_panel_rect, manager=ui_manager
    )
    ...
    preset_dropdown = pygame_gui.elements.UIDropDownMenu(
    ...
    reset_button = pygame_gui.elements.UIButton(
        relative_rect=pygame.Rect(
            ((UI_SIDEBAR_WIDTH - 30) // 2 + 20, y_pos), ((UI_SIDEBAR_WIDTH - 30) // 2, 30)
        ),
        text="Reset",
        manager=ui_manager,
        container=control_panel,
    ...
    gravity_label = pygame_gui.elements.UILabel(
        relative_rect=pygame.Rect(
            # Start label at 10.0x
            (10, y_pos),
            (UI_SIDEBAR_WIDTH - 20, 20),
        ),
        text="Gravity: 10.0x",
        manager=ui_manager,
        container=control_panel,
    ...
    y_pos += 30
    adaptive_button = pygame_gui.elements.UIButton(
        relative_rect=pygame.Rect((10, y_pos), (UI_SIDEBAR_WIDTH - 20, 30)),
        text=f"Adaptive Step: {
            'ON' if ADAPTIVE_STEPPING else 'OFF'}",
        manager=ui_manager,
        container=control_panel,
    )
    y_pos += 40
    ...
    y_pos += 25
    trails_button = pygame_gui.elements.UIButton(
        relative_rect=pygame.Rect((10, y_pos), ((UI_SIDEBAR_WIDTH - 30) // 2, 30)),
        text=f"Trails: {
            'ON' if SHOW_TRAILS else 'OFF'}",
        manager=ui_manager,
        container=control_panel,
    )
    gfield_button = pygame_gui.elements.UIButton(
        relative_rect=pygame.Rect(
            ((UI_SIDEBAR_WIDTH - 30) // 2 + 20, y_pos), ((UI_SIDEBAR_WIDTH - 30) // 2, 30)
        ),
        text=f"Grav Field: {'OFF'}",
        manager=ui_manager,
        container=control_panel,
    )
    y_pos += 40
    trail_len_label = pygame_gui.elements.UILabel(
        relative_rect=pygame.Rect((10, y_pos), (UI_SIDEBAR_WIDTH - 20, 20)),
    ...
    y_pos += 20
    trail_len_slider = pygame_gui.elements.UIHorizontalSlider(
        relative_rect=pygame.Rect((10, y_pos), (UI_SIDEBAR_WIDTH - 20, 20)),
    ...
    y_pos += 30
    boundaries_button = pygame_gui.elements.UIButton(
        relative_rect=pygame.Rect((10, y_pos), (UI_SIDEBAR_WIDTH - 20, 30)),
        text="Boundaries: ON",
        manager=ui_manager,
        container=control_panel,
    ...
    merge_button = pygame_gui.elements.UIButton(
        relative_rect=pygame.Rect((10, y_pos), (UI_SIDEBAR_WIDTH - 20, 30)),
        text=f"Merge: {
            'ON' if MERGE_ON_COLLISION else 'OFF'}",
        manager=ui_manager,
        container=control_panel,
    )
    y_pos += 40
    ...
    y_pos += 25
    center_button = pygame_gui.elements.UIButton(
        relative_rect=pygame.Rect((10, y_pos), ((UI_SIDEBAR_WIDTH - 30) // 2, 30)),
        text="Center View",
        manager=ui_manager,
        container=control_panel,
    )
    zoom_reset_button = pygame_gui.elements.UIButton(
        relative_rect=pygame.Rect(
            ((UI_SIDEBAR_WIDTH - 30) // 2 + 20, y_pos), ((UI_SIDEBAR_WIDTH - 30) // 2, 30)
        ),
        text="Reset Zoom",
    ...
    mass_label = pygame_gui.elements.UILabel(
        relative_rect=pygame.Rect((10, y_pos), (UI_SIDEBAR_WIDTH - 20, 20)),
        text=f"Mass: {
            mass_to_display(DEFAULT_NEXT_BODY_MASS)}",
        manager=ui_manager,
        container=control_panel,
    )
    y_pos += 20
    ...
    add_mass_slider = pygame_gui.elements.UIHorizontalSlider(
        relative_rect=pygame.Rect((10, y_pos), (UI_SIDEBAR_WIDTH - 20, 20)),
        start_value=mass_slider_default,
        value_range=(0.0, 1.0),
        manager=ui_manager,
        container=control_panel,
    )
    ...
    y_pos += 20
    add_radius_slider = pygame_gui.elements.UIHorizontalSlider(
        relative_rect=pygame.Rect((10, y_pos), (UI_SIDEBAR_WIDTH - 20, 20)),
    ...
    y_pos += 25
    selected_body_name_label = pygame_gui.elements.UILabel(
        relative_rect=pygame.Rect((10, y_pos), (UI_SIDEBAR_WIDTH - 20, 20)),
        text="Name: None",
        manager=ui_manager,
        container=control_panel,
    ...
    y_pos += 25
    selected_body_mass_label = pygame_gui.elements.UILabel(
        relative_rect=pygame.Rect((10, y_pos), (UI_SIDEBAR_WIDTH - 20, 20)),
        text="Mass: N/A",
        manager=ui_manager,
        container=control_panel,
    ...
    y_pos += 20
    selected_mass_slider = pygame_gui.elements.UIHorizontalSlider(
        relative_rect=pygame.Rect((10, y_pos), (UI_SIDEBAR_WIDTH - 20, 20)),
        start_value=0.5,
        value_range=(0.0, 1.0),
        manager=ui_manager,
    ...
    # Status Bar Text
    status_text_label = pygame_gui.elements.UILabel(
    ...
    help_window = pygame_gui.elements.UIWindow(
        rect=help_window_rect,
        manager=ui_manager,
...
<b>Physics Notes:</b><br>
 ...
 - Energy/Momentum calculations are approximate due to numerical integration and softening.
    ...
    help_textbox = pygame_gui.elements.UITextBox(
        html_text=help_text_content,
        relative_rect=pygame.Rect((0, 0), help_window.get_container().get_size()),
        manager=ui_manager,
    ...
    # Screen position where right-click started
    ...
    def load_preset(preset_name):
        ...
        # Use nonlocal for camera variables as well, since they are now defined
        # in main
        ...
        if com_pos is not None:
            screen_center = np.array(
                [(WIDTH - UI_SIDEBAR_WIDTH) / 2, (HEIGHT - UI_BOTTOM_HEIGHT) / 2]
            )
    ...
    # --- Main Game Loop ---
    while running:
        # Real time elapsed (seconds), capped at 60 FPS
        ...
        # Add small epsilon to zoom to prevent division by zero if zoom becomes
        # extremely small
        mouse_world_pos = (
            (mouse_screen_pos - current_pan) / (current_zoom + 1e-18) if not mouse_over_ui else None
        )
            ...
            if event.type == pygame.USEREVENT:
                if event.user_type == pygame_gui.UI_BUTTON_PRESSED:
                    ...
                    if event.ui_element == play_pause_button:
                        ...
                        # Instantly update current camera as well for immediate
                        # reset feel
                        current_zoom = target_zoom
                        current_pan = target_pan.copy()
                    ...
                    if event.ui_element == speed_slider:
                        ...
                        event.ui_element == selected_mass_slider
                    ):  # <<< Handle selected body mass slider >>>
                        if selected_body and not selected_body.fixed:
                            # Convert slider value (0-1) back to mass using log
                            # scale
                            log_mass = mass_min_log + event.value * (mass_max_log - mass_min_log)
                            new_mass = 10**log_mass
                            selected_body.mass = new_mass
                            # Update the label
                            selected_body_mass_label.set_text(
                                f"Mass: {mass_to_display(selected_body.mass)}"
                            )
            ...
            # --- Handle Mouse & Keyboard Input (if not over UI panel for relevant events) ---
                ...
                if not mouse_over_ui:
                    if event.button == 1:  # Left click
                        ...
                        for body in reversed(bodies):  # Check topmost first
                            ...
                            # Use scaled radius
                            select_radius_sq = (
                                max(5, body.radius_pixels * effective_zoom_scale)
                            ) ** 2
                            if (
                                np.sum((np.array(body_screen_pos) - mouse_screen_pos) ** 2)
                                < select_radius_sq
                            ):
                        ...
                        if clicked_on_body:
                            ...
                            # Offset from body center to mouse click (screen
                            # pixels)
                            ...
                            if not selected_body.fixed:
                                # Set slider value based on current mass (log
                                # scale)
                                current_log_mass = math.log10(max(1e-9, selected_body.mass))
                                slider_val = 0.5  # Default
                                if mass_max_log > mass_min_log:
                                    slider_val = (current_log_mass - mass_min_log) / (
                                        mass_max_log - mass_min_log
                                    )
                ...
                if event.button == 4:  # Mouse wheel up - Zoom in
                    if mouse_world_pos is not None:  # Need a world pos to zoom towards
                        ...
                        # Adjust pan to keep mouse pointer location static in
                        # world space
                    ...
                    if mouse_world_pos is not None:  # Need a world pos to zoom towards
                        target_zoom /= ZOOM_FACTOR
                        # Prevent zoom <= 0
                        target_zoom = max(1e-18, target_zoom)
                        target_pan = mouse_screen_pos - mouse_world_pos * target_zoom

            elif event.type == pygame.MOUSEBUTTONUP:
                # Handle mouse up regardless of UI hover for releasing
                # drags/adds
                if event.button == 1:  # Left click release
                    # Only clear status if we were dragging a body
                    if dragging_body:
                        status_text_label.set_text(
                            f"Released {
                                selected_body.name if selected_body else ''}"
                        )
                    ...
                    if adding_body_state == 1:
                        # Check if mouse is currently over UI - if so, cancel
                        # add
                        if mouse_over_ui:
                            ...
                            add_start_world = (add_body_start_screen - current_pan) / (
                                current_zoom + 1e-18
                            )
                            # End position (current mouse) in world coords
                            add_end_world = mouse_world_pos

                            # Calculate velocity from drag vector (world units
                            # -> m/s)
                ...
                # Let's allow dragging over UI for now, but panning stops if
                # mouse enters UI.
                if dragging_body and selected_body:  # Dragging selected body
                    new_body_center_screen = mouse_screen_pos - mouse_offset
                    selected_body.pos = (new_body_center_screen - current_pan) / (
                        current_zoom + 1e-18
                    )
                    if paused:
                        selected_body.vel = np.zeros(2)  # Reset velocity if paused
                    selected_body.clear_trail()  # Avoid trail jumps while dragging

                elif (
                    dragging_camera and not mouse_over_ui
                ):  # Panning camera only if mouse outside UI
        ...
        if not paused and bodies:
            ...
            world_bounds_sim = (
                get_world_bounds_sim(current_zoom, current_pan) if use_boundaries else None
            )
            ...
            if ADAPTIVE_STEPPING:
                ...
                # Record how much time actually passed
                ...
                # Fixed step always advances by proposed dt
            ...
            indices_to_remove = detect_and_handle_collisions(
                bodies, merge_on_collision=merge_on_collision
            )
        ...
        if SHOW_GRAV_FIELD:
            ...
            render_gravitational_field(
                screen, bodies, INITIAL_G * gravity_multiplier, current_zoom, current_pan
            )
        ...
        for body in bodies:
            ...
            # Show labels if zoomed in moderately
        ...
        if adding_body_state == 1:  # Draw preview even if mouse is over UI temporarily
            ...
            pygame.gfxdraw.aacircle(
                ...
                int(start_screen[0]),
                int(start_screen[1]),
                next_body_radius_pixels,
                color_options[color_index],
            )
            ...
            try:  # Add try-except for font rendering
                ...
                # Use last known valid mouse_world_pos if current is None (over
                # UI)
        ...
        if not paused:
            ...
            if (
                ...
                status_info = (
                    f"Time: {
                        time_to_display(simulation_time)} | "
                    f"Bodies: {
                        len(bodies)} | Step: {
                        time_step:.1f}s | "
                    f"Zoom: {
