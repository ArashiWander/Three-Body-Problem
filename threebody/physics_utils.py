...
# from .jit import compute_accelerations_jit  # not currently used but available
...
def adaptive_rk4_step(
    ...
    *,
    ...
    pos_half, vel_half = perform_rk4_step(bodies, dt / 2.0, g_constant,
                                          use_gpu=use_gpu)
    temp_bodies = [PhysicsBody(b.mass, pos_half[i], vel_half[i], fixed=b.fixed)
    ...
    if max_rel_error <= error_tolerance or dt <= C.MIN_TIME_STEP:
        for i, body in enumerate(bodies):
            ...
            if hasattr(body, "update_physics_state"):
                ...
                body.pos = pos2[i]
                body.vel = vel2[i]
            if (use_boundaries and bounds_sim is not None and
                    hasattr(body, "handle_boundary_collision")):
